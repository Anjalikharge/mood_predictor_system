...
def load_model():
    try:
        ...
        model_loader = ModelLoader()
        st.code(f"Base directory: {model_loader.base_dir}")
        st.code(f"Model directory: {model_loader.model_dir}")
        st.code(f"Current working directory: {os.getcwd()}")
        
        # Show available files
        if os.path.exists(model_loader.base_dir):
            files = os.listdir(model_loader.base_dir)
            st.code(f"Files in base directory: {files}")
